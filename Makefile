...
generate-ssh-keys: ## Generates SSH keys for the JWT library
	...
	U_ID=${UID} docker exec -it --user ${UID} ${DOCKER_BE} openssl genrsa -passout pass:d43023dc5a5b65445c3497f40d6b37dfa0b0f3f567c0ff2189003a3d6f935c68 -out config/jwt/private.pem -aes256 4096
	U_ID=${UID} docker exec -it --user ${UID} ${DOCKER_BE} openssl rsa -pubout -passin pass:d43023dc5a5b65445c3497f40d6b37dfa0b0f3f567c0ff2189003a3d6f935c68 -in config/jwt/private.pem -out config/jwt/public.pem